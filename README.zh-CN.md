...
- **IX 中转编排**（可选，默认关闭）—— 由 subagg 去 L4 中转平台建转发端口，
  ...
  原节点与 `IX_` 派生节点并存；不可用 IX 节点保留状态但不进入订阅。
...
打开 <http://127.0.0.1:8787>。独立的本地开发入口可以使用 `ADMIN_TOKEN`；
生产环境会拒绝这个兼容入口。

生产 Google 登录需要单独创建 Google Web OAuth Client，并设置
`APP_ENV=production`、`PUBLIC_BASE_URL` / `WEB_APP_URL`、`GOOGLE_CLIENT_ID`、
`GOOGLE_CLIENT_SECRET`、`SESSION_SECRET`、`SESSION_COOKIE_SECURE=true` 和
`ALLOW_DEV_LOGIN=false`。回调地址必须逐字注册为：

```text
https://你的域名/auth/google/callback
```

服务端只接受 `GOOGLE_ALLOWED_EMAILS` 里列出的已验证邮箱，生产环境恰好一个
（单用户自托管）。Google token 只在登录回调内使用后立即丢弃；本站只保存自己的
可撤销哈希会话。

开发时用 `npm run dev`（tsx 监听模式）。

> **在把它暴露到公网之前，请先读一遍 [SECURITY.md](./SECURITY.md)。**
> 数据库里存的是你的代理凭据。必须挂 HTTPS，每个管理接口请求都必须经过本站
> Google 应用会话保护。
...
可选功能。先生成明确的 IX 派生节点，再像普通节点一样选择和使用。
...
L4 转发不解 TLS、不看载荷。所以每个 IX 派生节点**只改拨号地址** —— `server` 与 `port`。
UUID、密码、加密方式、TLS、SNI、ALPN、REALITY、Host、path、gRPC serviceName、
flow 一律不动。`ix_port_mappings` 是唯一持久化来源，节点目录据此生成
`IX_<原节点名>`。派生指纹只由 provider 与原节点指纹决定，所以平台批量更换
入口域名或端口时，profile 选择和延迟历史都不会失效。
...
2. **测试连接** —— 拉回线路清单、每条线路的端口配额、流量、到期时间，
   以及一份明确写出"这个账号做不到什么"的清单。随后点「同步状态」，会自动发现
   并认领目标地址与本地节点精确匹配的现有端口。
3. **生成 IX 节点** —— 节点页勾选原节点 →「生成 IX 节点」
   （一批最多 50 个指纹）。若平台上已经有指向同一个 `host:port` 的端口，
   subagg 会**认领**它而不是重复创建 —— 配额小到不允许浪费。
4. **直接选择派生节点** —— 节点列表会同时显示原节点和 `IX_...` 节点。
   profile 直接选择 `IX_...` 节点，不再有 profile 级 IX 开关。
...
- **账号不给的话，只能用直连转发。** 链式转发、自定义转发出口、入站代理
  ...
  而"测试连接"会逐条列出你没有哪些能力。
- **有些节点会在创建前被拒绝。** 只要改地址会逼 subagg 去猜一个伪装参数，
  它就不会调用远端创建接口，也不会占配额：REALITY 没有显式 `sni`、Shadowsocks 带 obfs 插件、
...
**生成订阅的过程零出站请求。**控制面同步失败时保留最后一次入口，并把 IX 节点标成
“状态过期”。端口挂起、失效或不可用时，IX 节点继续显示为“不可用”，但不会进入订阅，
也绝不会顶着 `IX_` 名称偷偷变成原始直连节点。需要回退时，明确选择旁边保留的原节点。

删除 IX 节点时，如果它是远端端口的最后一个本地引用，系统会删除远端端口并回读确认；
中转商仍有 IX 节点时禁止直接删除，避免远端端口继续占配额却失去追踪。

### 两个延迟不是一回事，别相加

- **节点页**上的延迟由 subagg 对表格中显示的地址实测：原节点测原落地，
  `IX_` 节点测 IX 入口。
...
受影响的 IX 节点变成不可用、服务不崩 —— 但你得把凭据重新输一遍。

### 相关配置

| 环境变量 | 默认值 | 含义 |
|---|---|---|
| `IX_SYNC_INTERVAL_MINUTES` | `5` | 多久与平台对齐一次映射并刷新 IX 节点入口。**`0` 表示禁用后台同步**。上限 1440。 |
...
统一节点目录先生成「原节点 + IX 派生节点」，profile 对两类节点统一走
`filter → chain → emit`。
